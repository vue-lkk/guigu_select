...
**<6>.ref**:父组件获取子组件实例 VC,获取子组件的响应式数据以及方法

**<7>.slot**:插槽(默认插槽、具名插槽、作用域插槽)实现父子组件通信........

### 《《《**_1.1.props_**》》》

props 可以实现父子组件通信,在 vue3 中我们可以通过 defineProps 获取父组件传递的数据。
且在组件内部不需要引入 defineProps 方法可以直接使用！
...
子组件获取到 props 数据就可以在模板中使用了,但是**切记 props 是只读的(只能读取，不能修改)**

### 《《《**_1.2 自定义事件_**》》》

在 vue 框架中事件分为两种:一种是：**原生的 DOM 事件**，另外一种：**自定义事件**。
...
vue2 框架当中:这种写法自定义事件,可以通过.native 修饰符变为原生 DOM 事件
vue3 框架下面写法其实即为原生 DOM 事件
...
**_在 vue3 中一个组件可以通过使用多个 v-model_**,让父子组件多个数据同步,下方代码相当于给组件 Child 传递两个 props 分别是**pageNo**与**pageSize**，以及绑定两个自定义事件**update:pageNo**与**update:pageSize**实现父子数据同步
...
### 《《《**_1.7 provide 与 inject_**》》》

**provide[提供]**

**inject[注入]**

vue3 提供两个方法 provide 与 inject,可以实现 **隔辈组件传递参数**
...
//定义组合式API仓库
import { defineStore } from "pinia";
// 要写getters怎么办，就使用vue中的computed来代替
...
出现报错：（不允许使用var,说明配置成功）
5:1  error  Unexpected var, use let or const instead  no-var
5:5  error  'a' is assigned a value but never used    @typescript-eslint/no-unused-vars
5:8  error  Replace `1000;` with `·1000;⏎`            prettier/prettier
```
...
首先准备工作： 1.需要在当前本地项目中---初始化本地仓库 ：`git init` 出现`.git`文件夹说明初始化成功(可能隐藏) 2.创建远程仓库 gitee /github 3.关联仓库：`git remote add origin https://github.com/vue-lkk/guigu_select.git` 4.仓库暂存区：`git add .` 5.提交：`git commit -m "first commit` 6.将代码 push 到远程仓库：`git push -u origin master` 7.再次 push 到远程仓库：`git push`
...
在根目录创建`src/scritps/preinstall.js`文件，添加下面的内容
...
### 3.1 集成 element-plus  
**地址：https://element-plus.gitee.io/zh-CN/component/icon.html#icon-collection**
...
@element-plus/icons-vue 是安装 element-plus 中的图标库
...
2.使用（注意必须是 :icon ="Edit" 不要忘记写':'）
<el-button type="primary" :icon="Plus">主要按钮</el-button>
<el-button type="warning" :icon="Edit">编辑按钮</el-button>
<el-button type="danger" :icon="Delete">删除按钮</el-button>
```
...
通过**import.meta.env**获取环境变量,在人口文件（main.js）或者 在组件内部都可以使用
...
在阿里巴巴矢量图下载 svg 图标：创建 xxx.svg 文件将代码粘贴进去

```
assets/icons/moeny.svg

<svg t="1685082434062" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="927" width="200" height="200"><path d="M832 0H192a192 192 0 0 0-192 192v640a192 192 0 0 0 192 192h640a192 192 0 0 0 192-192V192a192 192 0 0 0-192-192z m128 832a128 128 0 0 1-128 128H192a128 128 0 0 1-128-128V192a128 128 0 0 1 128-128h640a128 128 0 0 1 128 128v640z" fill="#454545" p-id="928"></path><path d="M652.8 503.04a32 32 0 1 0 0-64H576l103.04-128a32.128 32.128 0 1 0-50.56-39.68L521.6 401.92 417.28 270.08a32.128 32.128 0 1 0-50.56 39.68l103.04 128H371.2a32 32 0 1 0 0 64h118.4v82.56H371.2a32 32 0 1 0 0 64h118.4v85.76a32 32 0 0 0 64 0v-85.76h99.2a32 32 0 1 0 0-64H553.6V503.04h99.2z" fill="#454545" p-id="929"></path></svg>
```

**注意：粘贴之后需要将 里面的 fill 直接干掉他或者改成 fill="#......"，不然修改 fill='pink'不生效**

```
<svg style="width: 30px;height:30px;" >
    <use xlink:href="#icon-moeny"  fill="pink"></use>
  </svg>
```
...
<template>
  <div>
    ...
    <svg :style="{ width: width, height: height }">
      // xlink:href执行使用哪一个图标,属性制务必是 #icon-图标名字，不重要写.svg后缀
...
```
<SvgIcon name="moeny" color="green" width="30" height="30" ></SvgIcon>
```
...
注意：**@import "./src/styles/variable.less";** 后面的`;`不要忘记，不然会报错!

在所有的地方都可以使用全局 scss 变量：
...
配置运行报错 require is not defined

```
去掉package.jon 中的 type:"module" 即可
```
