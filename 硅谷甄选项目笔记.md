...
子组件获取到props数据就可以在模板中使用了,但是**切记props是只读的(只能读取，不能修改)**
...
在vue框架中事件分为两种:一种是：**原生的DOM事件**，另外一种：**自定义事件**。
...
我们会发现在script标签内部,使用了 **defineEmits方法**，此方法是vue3提供的方法,不需要引入直接使用。defineEmits方法执行，传递一个数组，数组元素即为将来组件需要触发的自定义事件类型，此方执行会返回一个$emit方法用于触发自定义事件。
...
***在vue3中一个组件可以通过使用多个v-model***,让父子组件多个数据同步,下方代码相当于给组件Child传递两个props分别是**pageNo**与**pageSize**，以及绑定两个自定义事件**update:pageNo**与**update:pageSize**实现父子数据同步
...
vue3提供两个方法provide与inject,可以实现 **隔辈组件传递参数**
...
首先准备工作：
1.需要在当前本地项目中---初始化本地仓库 ：`git init`   出现`.git`文件夹说明初始化成功(可能隐藏)
2.创建远程仓库 gitee /github 
3.关联仓库：`git remote add origin https://github.com/vue-lkk/guigu_select.git`
4.仓库暂存区：`git add .`
5.提交：`git commit -m "first commit`
6.将代码push到远程仓库：`git push -u origin master`
7.再次push到远程仓库：`git push`
...
@element-plus/icons-vue 是安装element-plus中的图标库
```
...
这对页面性能来说是个很大的提升，而且我们**SVG文件比img要小的很多**，放在项目中几乎不占用资源。
...
```
在阿里巴巴矢量图下载svg图标：创建 xxx.svg文件将代码粘贴进去
```
...
```
**注意：粘贴之后需要将 里面的fill直接干掉他或者改成 fill="#......"，不然修改fill='pink'不生效**
